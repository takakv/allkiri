...
//!
//! ```sh
//! cargo run --features pkcs11 --example sign_id_card -- test.txt test.asice
//! ```
//!
//! The issuing CA is discovered through the signing certificate's AIA `caIssuers` URL.
//! Alternatively, pass the certificate as an explicit third argument.
//!
//! ```sh
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (input, output, ca_path) = match args.as_slice() {
        [input, output] => (input, output, None),
        [input, output, ca_path] => (input, output, Some(ca_path)),
        _ => {
            eprintln!("usage: sign_id_card <file> <out.asice> [issuer.(pem|der)]");
            std::process::exit(2);
        }
    };
